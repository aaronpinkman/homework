...
int main()
{
	...
	printf("\n2.请输入数字选择运算符号（1.加法    2.减法    3.乘法    4.除法    5.混合运算）：");
	scanf("%d", &fuhao);
	while (fuhao < 1 || fuhao > 5)
	{
		printf("输入的数字必须在1到5之间！\n");
		scanf("%d", &fuhao);
	}
	printf("\n3.请输入数字设置练习的最大数（如十以内、百以内等,但不能小于1）：");
	scanf("%d", &max);
	while (max < 1)
	{
		printf("输入的数字非法！\n");
	...
	printf("\n5.请输入数字设置是否有括号（1.无   2.有）备注：此选项仅混合运算时选择，其他选项选1即可 ：");
	scanf("%d", &kuohao);
	...
	printf("\n6.请输入数字设置输出方式（1.直接显示在屏幕上   2.输出到文件） ：");
	scanf("%d", &file);
	while (file != 1 && file != 2)
	{
		printf("输入的数字非法！\n");
